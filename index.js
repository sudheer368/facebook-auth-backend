...
// ---------------- GET TOKENS FROM CLOUD FUNCTION ----------------
async function getDeviceTokens(companyId) {
  try {
    const url = `https://us-central1-kiran-interior-b7e9c.cloudfunctions.net/Interiorleadsnotification/users?companyId=${companyId}`;
...
async function sendNotification(appConfig, leadData) {
  ...
  try {
    ...
    if (!tokens || tokens.length === 0) {
      log(`[${appName}] No tokens found for company ${companyId}`);
      return;
    }
    ...
    try {
      ...
      log(`[${appName}] Failed to get access token: ${authError.message}`);
      return;
    }
    ...
    results.forEach((result, index) => {
      if (result.status === "rejected") {
        ...
        const errorMsg = result.reason?.response?.data?.error?.message || result.reason?.message || 'Unknown error';
        log(`[${appName}] Failed to send to token ${token.substring(0, 20)}...: ${errorMsg}`);
      }
    });

  } catch (err) {
    log(`[${appName}] Notification failed: ${err.message}`);
    console.error(`[${appName}] Notification error details:`, err);
  }
}

// ---------------- CALL NOW FUNCTION ----------------
async function callNow(appConfig, leadData) {
  const numberId = appConfig.numberId;
  const appName = appConfig.name;
  ...
  app.post(`/webhook/${appConfig.name}`, async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        savedLeadId = responseData?.id || null;
        ...
        const leadDataForNotification = {
          name: payload.name,
          phone: payload.phone,
          leadId: savedLeadId || payload.leadId,
          email: payload.email,
          city: payload.city,
          spaceType: payload.spaceType,
          timeline: payload.timeline,
        ...
        try {
          ...
          log(`[${appConfig.name}] ❌ Notification also failed: ${notifyErr.message}`);
        }
      }
